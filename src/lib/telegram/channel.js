...
let deal = async (ctx) => {
	...
	const resourceUrl = `https://t.me/s/${username}`;

	const data = await fetch(resourceUrl).then((res) => res.text());
	...
	const feedTitle = channelName + ' - Telegram Channel';
		...
		items:
			...
			list
				.map((index, item) => {
					...
					if (
						item.find(
							'.tgme_widget_message_author .tgme_widget_message_via_bot,.tgme_widget_message_forwarded_from .tgme_widget_message_via_bot'
						).length
					) {
					...
					const messageMedia = generateMedia(
						'.tgme_widget_message_photo_wrap,.tgme_widget_message_service_photo,.tgme_widget_message_sticker,.tgme_widget_message_tgsticker,.tgme_widget_message_video_player'
					);
					...
					const location = () => {
						...
						if (locationObj.length) {
							...
							const mapImgHtml = mapBackgroundUrlSrc
								? `<img src="${mapBackgroundUrlSrc}">`
								: showMediaTagAsEmoji
								? mediaTagDict[LOCATION][1]
								: mediaTagDict[LOCATION][0];
					...
					const linkPreview = () => {
						...
						if (linkPreviewTitleObj.length) {
							linkPreviewTitle = `<b><a href="${item
								.find('.tgme_widget_message_link_preview')
								.attr('href')}">${linkPreviewTitleObj.text()}</a></b><br>`;
						} else {
							linkPreviewTitle = '';
						}
						...
						if (
							linkPreviewSite.length > 0 ||
							linkPreviewTitle.length > 0 ||
							linkPreviewDescription.length > 0 ||
							linkPreviewImage.length > 0
						) {
					...
					if (documentWrapObj.length) {
						documentWrapObj.each((_, wrap) => {
							...
							const _attachmentHtml =
								(documentTitle ? `<p><b>${documentTitle}</b></p>` : '') + (documentExtra ? `<p><small>${documentExtra}</small></p>` : '');
					...
					const contactHtml =
						contactNameHtml || contactPhoneHtml
							? `<p>${contactNameHtml}${contactName && contactPhone ? ': ' : ''}${contactPhoneHtml}</p>`
							: '';
					...
					if (msgTypes.includes(UNSUPPORTED)) {
						if (unsupportedNodes.length) {
							...
							unsupportedNodes.find('.message_media_view_in_telegram').each(function () {
								const $this = $(this);
								unsupportedHtml += $this.attr('href')
									? `<p><a href="${$this.attr('href')}">${$this.text()}</a></p>`
									: `<p>${$this.text()}</p>`;
							});
							unsupportedHtml += '</blockquote>';
						} else {
					...
					if (showMediaTagInTitle) {
						msgTypes.forEach((type) => {
							if (
								...
								type !== FORWARDED ||
								type !== VIA_BOT ||
								(type === REPLY && showReplyTo) ||
								(type === FORWARDED && showFwdFrom) ||
								(type === VIA_BOT && showViaBot)
							) {
					...
					const messageTextObj = item.find(
						`.${
							msgTypes.includes(PARTIALLY_UNSUPPORTED) ? 'media_supported_cont' : 'tgme_widget_message_bubble'
